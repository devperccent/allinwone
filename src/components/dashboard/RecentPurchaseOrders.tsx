    ...
    <div className="rounded-xl border border-border bg-card">
      <div className="flex items-center justify-between p-4 border-b border-border">
        ...
        <Button variant="ghost" size="sm" asChild className="gap-1 text-muted-foreground hover:text-foreground">
          <Link to="/purchase-orders">View all<ArrowRight className="w-4 h-4" /></Link>
        </Button>
      </div>
      <div className="divide-y divide-border">
        {purchaseOrders.length === 0 ? (
          ...
          purchaseOrders.map((po) => (
            <Link key={po.id} to={`/purchase-orders/${po.id}`} className="flex items-center justify-between p-3 hover:bg-muted/30 transition-colors gap-3">
              <div className="min-w-0">
                <p className="font-medium truncate">{po.po_number}</p>
                <p className="text-sm text-muted-foreground truncate">{po.supplier_name}</p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <Badge className={cn('text-xs hidden sm:inline-flex', statusConfig[po.status]?.className)}>{statusConfig[po.status]?.label || po.status}</Badge>
